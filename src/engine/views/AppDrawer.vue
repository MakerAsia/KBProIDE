<template>
  <v-navigation-drawer
    ...
    <v-toolbar color="primary darken-1" dark>
      <img v-bind:src="computeLogo" height="36" alt="Bloccoly : Programming IDE for everyone">
      <v-toolbar-title class="ml-0 pl-3">
        <span class="hidden-sm-and-down headline">Bloccoly</span>
      </v-toolbar-title>        
    </v-toolbar>
      ...
      <v-list dense expand>
        <template v-for="(item, i) in menus">
            ...
            <v-list-tile v-else :to="!item.href ? { name: item.name } : null" :href="item.href" ripple="ripple" :disabled="item.disabled" :target="item.target" rel="noopener" :key="item.name">
              <v-list-tile-action v-if="item.icon" @click.stop="drawer2 = !drawer2">
...
export default {
  ...
  computed: {
    ...
    computeLogo () {
      return '/static/logo/broccoli.png';
    },
